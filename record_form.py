...
def new_record_start(master, sys_name, petrol_p, diesel_p):
    global new_re_state
    new_re_state = True
    global rec_win
    ...
    rec_win.geometry('400x480')
    rec_win.title(f'New Record - {sys_name.upper()}')
...
def new_record_searched(reg_num, data_base, cursor):
    ...
    if len(result) == 1:
        # Vehicle Type
        Label(mini_frame, text='Vehicle Type : ', font=('Arial', 10, 'bold'), bg='#DBDBDB').grid(row=0, column=0, pady=5)
        veh_typo_final = Entry(mini_frame, font=('Arial', 10, 'bold'), width=25)
        veh_typo_final.grid(row=0, column=1)
        veh_typo_final.insert(END, result[0][1])

        # Page Number
        Label(mini_frame, text="Page No : ", font=('Arial', 10, 'bold'), bg='#DBDBDB').grid(row=1, column=0, pady=5, sticky=E)
        ...
        # Department
        dep_var = StringVar()
        dep_list = ['.....', 'Engineering', 'Health']
        dep_var.set(".....")
        Label(mini_frame, text="Department : ", font=('Arial', 10, 'bold'), bg='#DBDBDB').grid(row=2, column=0, sticky=E)
        dep_options = OptionMenu(mini_frame, dep_var, *dep_list)
        dep_options.config(width=15)
        dep_options.grid(row=2, column=1)
...
def record_submission(sys_name):
    ...
    if len(validate_results) == 1:
        ...
        if answer:
            ...
            cursor.execute(f"INSERT INTO oil_records(month, vehicle_reg, pg_num, fuel_price, fuel_l, r_price, department) VALUES("
                           f"'{month_value}',"
                           f"'{regis_num}',"
                           f"'{pg_num_final.get()}',"
                           f"'{unit}',"
