...
$month_sin = array('ජනවාරි', 'පෙබරවාරි', 'මාර්තු', 'අප්‍රිල්', 'මැයි', 'ජුනි', 'ජුලි', 'අගොස්තු', 'සැප්තැම්බර්', 'ඔත්තොඹර්', 'නොවැම්ඹර්', 'දෙසැම්ඹර්');
if(isset($_POST['arrow_btn'])){
	$month_num = $_POST['m_num'];
	$sign = $_POST['arrow_btn'];
	if($sign == "+" && $month_num < 12){
		$month_num += 1;
	}elseif($sign == "-" && $month_num > 1){
		$month_num -= 1;
	}
}else{
	$month_num = date('m');
}
$c_month = $month_list[$month_num-1];
$sin_month = $month_sin[$month_num-1];
...
<body>
<h1>නාගරික ඉංජිනේරු දෙපාර්තමේන්තුව - 2022 <?php echo $sin_month?> මස ඉන්ධන වාර්තාව.</h1>
<table border=3, cellspacing=0>
<tr>
<th rowspan='2'>අනු අංකය</th>
<th rowspan='2'>පිටු අංකය</th>
<th rowspan='2'>වාහන අංකය</th>
<th rowspan='2' width='40%'>වාහන වර්ගය</th>
...
if($count == true){
	...
	while($veh_row = $result_veh->fetch_assoc()){
		...
		$sql_oil = "SELECT * FROM oil_records WHERE month='$c_month' AND vehicle_reg='$reg_num'";
